...
complementary organizational app, Labor Supply & Demand (own repo). Collapsing "what a project
needs" and "who's actually being given to it" into one app is exactly the shape of mistake this
ecosystem has already learned from once (BurnedValue tried to be the plan and the execution and
the everything-else in one place); Good Plan stays the plan.
...
class DemandLine(db.Model):
    ...
    commit it, belongs to Labor Supply & Demand). Multiple lines for the same role are normal — demand for a
